...
let correctAnswer = 2; // Cambia esto según cuál es la respuesta correcta
...
function startGame() {
    ...
    window.location.href = 'URL_DE_DESTINO'; // Cambia esto por el enlace donde quieres redirigir
}
...
function checkAnswer() {
    if (selectedButton) {
        ...
        if (selectedVagon === correctAnswer) {
            alert("¡Correcto! Has seleccionado el vagon pocos objetos.");
        } else {
            alert("Incorrecto, en este vagon hay muchos objetos.");
        }
    } else {
...
document.getElementById('next').addEventListener('click', () => {
    window.location.href = 'juegotren3.html'; // Cambia 'paginaSiguiente.html' por la URL deseada
});

document.getElementById('retry').addEventListener('click', () => {
    window.location.href = 'juego2tren.html'; // Cambia 'paginaReintentar.html' por la URL deseada
});
